            ...
            <div id="bank-content" class="tab-content">
                @if(!empty($virtualAccounts) && count($virtualAccounts) > 0)
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach($virtualAccounts as $index => $account)
                        <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-6 border border-blue-200">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-semibold text-blue-900">{{ $account['bank_name'] }}</h3>
                                <i class="fas fa-university text-blue-600 text-2xl"></i>
                            </div>
                            <div class="space-y-3 mb-4">
                                <div>
                                    <p class="text-sm text-blue-700 font-medium">Account Number</p>
                                    <p class="text-lg font-bold text-blue-900" id="account-{{ $index }}">{{ $account['account_number'] }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-blue-700 font-medium">Account Name</p>
                                    <p class="text-blue-900 font-medium">{{ $account['account_name'] }}</p>
                                </div>
                            </div>
                            ...
                            <button onclick="copyToClipboard('account-{{ $index }}')" class="w-full bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors">
            ...
            <div id="card-content" class="tab-content hidden">
                ...
                <div class="max-w-md mx-auto">
                    ...
                    <form id="paystack-form" class="space-y-6">
                        ...
                        <input type="hidden" id="paystack-charges" value="{{ $paystackCharges ?? '1.5' }}">
...
$(document).ready(function() {
    ...
    $('#paystack-form').submit(function(e) {
        ...
        $.ajax({
            url: '{{ route("wallet.paystack.initialize") }}',
            method: 'POST',
            data: {
...
function generateVirtualAccount() {
    ...
    $.ajax({
        url: '{{ route("wallet.generate-virtual-account") }}',
        method: 'POST',
        data: {
            _token: $('meta[name="csrf-token"]').attr('content')
        },
