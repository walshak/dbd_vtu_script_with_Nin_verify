                ...
                <div class="mb-8">
                    <div class="bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl shadow-lg p-8 text-white relative overflow-hidden">
                        <div class="relative z-10">
                            <h1 class="text-3xl font-bold mb-2">Welcome back, {{ $firstName }}!</h1>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900">₦{{ number_format($user->wallet_balance ?? 0, 2) }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ $transactionSummary['total_transactions'] }}</p>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900">{{ $user->account_type_name }}</p>
                                <p class="text-xs text-purple-600 mt-1">
                                    <i class="fas fa-user-tag mr-1"></i>{{ $user->registration_status_name }}
                                </p>
                            </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-gray-900">₦{{ number_format($transactionSummary['total_spent'], 2) }}</p>
                                <p class="text-xs text-orange-600 mt-1">
                                    <i class="fas fa-credit-card mr-1"></i>{{ $user->created_at->diffForHumans() }}
                                </p>
                            </div>
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div class="space-y-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-sm font-medium">{{ $user->phone ?? 'N/A' }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                ...
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $user->isActive() ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                        {{ $user->registration_status_name }}
                                    </span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600">Referral Wallet</span>
                                    <span class="text-sm font-medium">₦{{ number_format($user->referral_wallet ?? 0, 2) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <div class="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                                        <div class="flex-shrink-0">
                                            <div class="w-10 h-10 rounded-full {{ $transaction->status_badge_class }} flex items-center justify-center">
                                                <i class="{{ $transaction->service_icon }} text-sm"></i>
                                            </div>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-medium text-gray-900 truncate">{{ $transaction->servicename }}</p>
                                        ...
                                        <div class="flex-shrink-0 text-right">
                                            ...
                                            <p class="text-xs {{ $transaction->status == 0 ? 'text-green-600' : 'text-red-600' }}">
...
$(document).ready(function() {
    // Service button clicks - let default link behavior work
    // Remove the e.preventDefault() that was blocking the links

    // Fund wallet button - already has href attribute, so no custom handler needed
