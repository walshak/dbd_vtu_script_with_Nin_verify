...
<div class="container mx-auto px-6 py-8">
    ...
    <div class="mb-6 bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-lg">
        <div class="flex items-start">
            ...
            <div class="ml-3">
                <h3 class="text-sm font-medium text-yellow-800">Electricity Provider Pricing</h3>
                <p class="mt-1 text-sm text-yellow-700">
                    Electricity providers are manually configured. The sync button updates provider availability status from Uzobest API.
                    Set selling prices based on your preferred markup percentage.
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-800" id="activeProvidersCount">{{ $statistics['active_providers'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-green-500">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-800">{{ $statistics['today_transactions'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-cyan-500">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-800">₦{{ number_format($statistics['today_revenue'], 2) }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border-l-4 border-yellow-500">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-800">₦{{ number_format($statistics['total_profit'], 2) }}</p>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200" id="providersTable">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $provider->eId }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <img src="{{ $provider->logo_path }}" alt="{{ $provider->ePlan }}"
                                             class="rounded-full mr-3" width="30" height="30">
                                        <strong class="text-sm font-medium text-gray-900">{{ strtoupper($provider->ePlan) }}</strong>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $provider->eStatus ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                        {{ $provider->eStatus ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-blue-600">
                                        ₦{{ number_format($provider->cost_price ?? $provider->eBuyingPrice, 2) }}/kWh
                                    </div>
                                    <div class="text-xs text-gray-500">From Uzobest</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-green-600">
                                        ₦{{ number_format($provider->selling_price ?? $provider->ePrice, 2) }}/kWh
                                    </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $provider->updated_at ? $provider->updated_at->format('M d, Y H:i') : 'N/A' }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <button onclick="editProvider({{ $provider->eId }})"
                                                class="text-blue-600 hover:text-blue-900 transition-colors duration-150"
                                                title="Edit Provider">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button onclick="toggleProvider({{ $provider->eId }})"
                                                class="text-{{ $provider->eStatus ? 'yellow' : 'green' }}-600 hover:text-{{ $provider->eStatus ? 'yellow' : 'green' }}-900 transition-colors duration-150"
                                                title="{{ $provider->eStatus ? 'Disable' : 'Enable' }} Provider">
                                            <i class="fas fa-{{ $provider->eStatus ? 'pause' : 'play' }}"></i>
                                        </button>
                                        <button onclick="deleteProvider({{ $provider->eId }}, '{{ $provider->ePlan }}')"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-white border border-gray-200 rounded-lg p-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                       id="electricityCharges" name="electricity_charges" step="0.01" value="{{ $settings['electricity_charges'] ?? 50 }}">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                       id="minimumAmount" name="minimum_amount" value="{{ $settings['minimum_amount'] ?? 1000 }}">
                            </div>

                            <div class="mb-4">
                                <label for="maximumAmount" class="block text-sm font-medium text-gray-700 mb-2">Maximum Amount (₦)</label>
                                <input type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                       id="maximumAmount" name="maximum_amount" value="{{ $settings['maximum_amount'] ?? 50000 }}">
                            </div>
                            ...
                            <div class="mb-4">
                                <label class="flex items-center cursor-pointer">
                                    <input type="checkbox" id="serviceEnabled" name="service_enabled"
                                           ...
                                           {{ ($settings['service_enabled'] ?? true) ? 'checked' : '' }}>
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                       id="agentDiscount" name="agent_discount" step="0.01" value="{{ $settings['agent_discount'] ?? 1 }}">
                            </div>

                            <div class="mb-4">
                                <label for="vendorDiscount" class="block text-sm font-medium text-gray-700 mb-2">Vendor Discount (%)</label>
                                <input type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                       id="vendorDiscount" name="vendor_discount" step="0.01" value="{{ $settings['vendor_discount'] ?? 2 }}">
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                          id="maintenanceMessage" name="maintenance_message" rows="3">{{ $settings['maintenance_message'] ?? 'Electricity service is temporarily unavailable. Please try again later.' }}</textarea>
                            </div>
                        ...
                        <div class="bg-white border border-gray-200 rounded-lg p-6">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="url" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                       id="apiUrl" name="electricity_api_url" value="{{ $apiConfig['electricity_api_url'] ?? '' }}">
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex">
                                    <input type="password" class="flex-1 px-3 py-2 border border-gray-300 rounded-l-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                           id="apiKey" name="electricity_api_key" value="{{ $apiConfig['electricity_api_key'] ?? '' }}">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="url" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                       id="validationUrl" name="electricity_validation_url" value="{{ $apiConfig['electricity_validation_url'] ?? '' }}">
                            </div>
...
function submitProvider(e) {
    ...
    fetch('{{ route("admin.electricity.providers.store") }}', {
        method: 'POST',
        body: formData,
        headers: {
...
function editProvider(providerId) {
    fetch(`{{ url('/admin/electricity/providers') }}/${providerId}`)
    .then(response => response.json())
    .then(data => {
        if (data.success) {
...
function toggleProvider(providerId) {
    if (confirm('Are you sure you want to change the status of this provider?')) {
        fetch(`{{ url('/admin/electricity/providers') }}/${providerId}/toggle`, {
            method: 'POST',
            headers: {
...
function syncProviders() {
    if (confirm('This will sync providers from the API. Continue?')) {
        fetch('{{ route("admin.electricity.sync-providers") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
function saveSettings(e) {
    ...
    fetch('{{ route("admin.electricity.settings.update") }}', {
        method: 'POST',
        body: formData,
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
...
function saveApiConfig(e) {
    ...
    fetch('{{ route("admin.electricity.api-config.update") }}', {
        method: 'POST',
        body: formData,
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
