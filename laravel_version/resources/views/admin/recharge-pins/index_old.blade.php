...
@section('page-title', 'Recharge Pin Discounts')
            ...
            <div class="box-body">
                <div class="table-responsive">
                    <table id="rechargePinTable" class="table table-sm table-bordered table-striped">
                        ...
                        <tbody>
                            @php $cnt = 1; @endphp
                            @foreach($rechargePinDiscounts as $discount)
                            <tr>
                                <td>
                                    <input type="checkbox" class="discount-checkbox" value="{{ $discount->aId }}">
                                </td>
                                <td>{{ $cnt++ }}</td>
                                <td>
                                    <strong>{{ $discount->network ? strtoupper($discount->network->network) : 'Unknown' }}</strong>
                                    @if($discount->network)
                                        <br><small class="text-muted">ID: {{ $discount->network->nId }}</small>
                                    @endif
                                </td>
                                <td>
                                    <span class="badge bg-primary">{{ number_format($discount->aUserDiscount, 1) }}%</span>
                                </td>
                                <td>
                                    <span class="badge bg-success">{{ number_format($discount->aAgentDiscount, 1) }}%</span>
                                </td>
                                <td>
                                    <span class="badge bg-warning">{{ number_format($discount->aVendorDiscount, 1) }}%</span>
                                </td>
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-primary" onclick="editAirtimeDiscount({{ $discount->aId }})">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                        <button class="btn btn-info" onclick="viewPricing({{ $discount->aId }})">
                                            <i class="fa fa-calculator"></i>
                                        </button>
                                        <button class="btn btn-danger" onclick="deleteDiscount({{ $discount->aId }})">
