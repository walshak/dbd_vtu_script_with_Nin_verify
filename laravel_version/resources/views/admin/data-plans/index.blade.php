...
@section('title', 'Data Plans Management')
        ...
        <div class="overflow-x-auto">
            <table id="dataPlansTable" class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @php $cnt = 1; @endphp
                    @forelse($dataPlans as $plan)
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <input type="checkbox" class="plan-checkbox rounded border-gray-300 text-blue-600 focus:ring-blue-500" value="{{ $plan->dId }}">
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $cnt++ }}</td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900">{{ $plan->dPlan }}</div>
                            <div class="text-sm text-gray-500">
                                {{ $plan->network->network ?? 'N/A' }} {{ $plan->dGroup }}
                                ({{ $plan->dValidity }})
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $plan->dPlanId }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-blue-600">
                                ₦{{ number_format((float)($plan->cost_price ?? $plan->dAmount ?? 0), 2) }}
                            </div>
                            <div class="text-xs text-gray-500">From Uzobest</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-green-600">
                                ₦{{ number_format((float)($plan->selling_price ?? $plan->userPrice ?? 0), 2) }}
                            </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <button onclick="editDataPlan({{ $plan->dId }})"
                                        class="text-blue-600 hover:text-blue-900 transition-colors duration-150">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <button onclick="togglePlanStatus({{ $plan->dId }})"
                                ...
                                <button onclick="deletePlan({{ $plan->pId }})"
...
function submitDataPlan() {
    ...
    fetch('{{ route("admin.data-plans.store") }}', {
        method: 'POST',
        body: formData,
        headers: {
...
function togglePlanStatus(planId) {
    fetch(`{{ url('/admin/data-plans') }}/${planId}/toggle-status`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
            'Content-Type': 'application/json'
...
function performBulkUpdate() {
    ...
    fetch('{{ route("admin.data-plans.bulk-update-prices") }}', {
        method: 'POST',
        body: formData,
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function syncDataPlans() {
    ...
    $.ajax({
        url: '{{ route("admin.data-plans.sync") }}',
        method: 'POST',
        headers: {
...
function exportPlans() {
    window.location.href = '{{ route("admin.data-plans.export") }}?format=csv';
}
</script>
@endpush
