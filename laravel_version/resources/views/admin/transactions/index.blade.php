                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900">{{ $transaction->transref ?? $transaction->tId }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                ...
                                <div class="ml-3">
                                    ...
                                    <div class="text-sm text-gray-500">{{ $transaction->user->phone ?? 'N/A' }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">{{ ucfirst($transaction->servicename ?? 'N/A') }}</div>
                            <div class="text-sm text-gray-500">{{ Str::limit($transaction->servicedesc ?? '', 30) }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php
                                $statusValue = $transaction->status ?? 0;
                                $statusText = $statusValue == 1 ? 'successful' : ($statusValue == 2 ? 'failed' : 'pending');
                                $statusClasses = [
                                    'successful' => 'bg-green-100 text-green-800',
                                    'failed' => 'bg-red-100 text-red-800',
                                    'pending' => 'bg-yellow-100 text-yellow-800',
                                ];
                                $statusClass = $statusClasses[$statusText] ?? 'bg-gray-100 text-gray-800';
                            @endphp
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $transaction->date ? (is_string($transaction->date) ? \Carbon\Carbon::parse($transaction->date)->format('M d, Y H:i') : $transaction->date->format('M d, Y H:i')) : 'N/A' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                <button onclick="viewTransaction('{{ $transaction->tId }}')"
                                        class="text-blue-600 hover:text-blue-900 transition-colors duration-150">
                                    <i class="fas fa-eye"></i>
                                </button>
                                <button onclick="retryTransaction('{{ $transaction->tId }}')"
                                        class="text-green-600 hover:text-green-900 transition-colors duration-150">
                                    <i class="fas fa-redo"></i>
                                </button>
                                <button onclick="refundTransaction('{{ $transaction->tId }}')"
